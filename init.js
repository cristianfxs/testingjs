...
(function () {
    ...
    */
    ...
    FXStreet.Resource.EventHubTokenUrl = '';
    FXStreet.Resource.EventHubNamespaceUrl = '';
    FXStreet.Resource.EventHubLoggerConfig = {};
    ...
    FXStreet.Util.initObject = function (createObject) {
        ...
        if (objectType === 'AdvertisePopup'
                && (domain.endsWith(".es") || domain.endsWith(".tr"))) {
            return;
        }
        ...
        if (FXStreet.Class[objectType]) {
            try{
                ...
                if (objectType === 'AdvertisePopup' && popupRetry < 3) {
                    popupRetry++;
    ...
    FXStreet.Util.initLogger = function () {
        var eventHubLoggerConfig = JSON.parse(FXStreet.Resource.EventHubLoggerConfig);

        FXStreet.Log = FXStreetLogger.Log.getInstance({
            tokenUrl: FXStreet.Resource.AuthorizationUrl,
            eventHub: {
                tokenUrl: FXStreet.Resource.EventHubTokenUrl,
                namespaceUrl: FXStreet.Resource.EventHubNamespaceUrl,
                entityConfigName: eventHubLoggerConfig.EntityConfigName,
                name: eventHubLoggerConfig.Name,
                publisher: eventHubLoggerConfig.Publisher
            }
        });
    };
...
(function () {
    FXStreet.Util.ready = function () {
        ...
        fillUserDataRepository();
    };

    var fillUserDataRepository = function () {
        if (FXStreet.Resource.UserInfo && FXStreet.Resource.UserInfo.IsLogged) { 
            var userService = FXStreet.Class.User.Service.getInstance();
            userService.fillUserRepositoryData(FXStreet.Resource.UserInfo.Email);
        } 
    };
    ...
    FXStreet.Util.preReady = function () {
        ...
        var responsiveDesign = FXStreet.Util.createObject("ResponsiveDesign", {}, "ready");
        FXStreet.Util.initObject(responsiveDesign);

        var menusSubscriberJson = FXStreet.Util.createObject("MenusSubscriber", {}, "ready");
        FXStreet.Util.initObject(menusSubscriberJson);

        var imagePopup = FXStreet.Util.createObject("PopupImages", {}, "ready");
        FXStreet.Util.initObject(imagePopup);


        FXStreet.Util.registerDynamicObjs();
        FXStreet.Util.initLogger();
    ...
    FXStreet.Util.GetMomentDateWithCurrentTimezone = function(dateString) {
        var dateMoment = dateString ? moment(dateString) : moment();

        var utcOffset = FXStreet.Class.Patterns.Singleton.TimeZoneManager.Instance().TimeZone().HoursUtcOffset;
        var hourToSet = dateMoment.utc().hour() + utcOffset;
        var minutesToAdd = 60 * (hourToSet % 1);
        var date = dateMoment.utc().hour(hourToSet).add(minutesToAdd, "minutes");
    ...
    */
    ...
    FXStreet.Util.dateToDayAndTimeString = function (dateInUtc) {
        ...
        var monthIndex = dateUTCTrick.getMonth(),
        ...
        result = monthsArray[monthIndex] + " " + day + ", " + time;
        return result;
    };
    ...
    FXStreet.Util.UserIsFromFxs = function (userInfo) {
        if (typeof userInfo === 'undefined' || userInfo === null || typeof userInfo.Email !== 'string' || userInfo.Email === null) {
            return false;
        }

        var result = userInfo.Email.toLowerCase().endsWith('@fxstreet.com');
        return result;
    }
}());
(function () {
...
(function () {
    FXStreet.Class.AdvertiseManager = (function () {
        var instance;
        ...
        var advertiseManager = function () {
            ...
            _this.Count = function (groupKey) {
                if (!slots[groupKey]) {
                    return 0;
                }
                else return slots[groupKey].length;
            };
    ...
    FXStreet.Class.AdvertiseBase = function () {
        ...
        _this.setVars = function () {
            ...
            _this.Labels.push(
                { Key: "url", Value: url },
                { Key: "encUrl", Value: encodeURIComponent(url) }
            );

            var atgCookie = _this.getCookie("atg");
            if (atgCookie !== "") {
                atgCookie.split('!').forEach(function (value, index, array) {
                    _this.Labels.push({ Key: value.split('.')[0], Value: value.split('.')[1] });
                });
            }

            _this.Async = FXStreet.Global.AsyncDfp;

            _this.AdvertiseManagerObj = FXStreet.Class.AdvertiseManager.Instance();
        };
        ...
        _this.renderAdvertise = function (onAdvertiseRendered) {
            ...
            if (_this.GroupKey && _this.AdvertiseManagerObj.Count(_this.GroupKey) < 2) {
                return;
            }
        ...
        _this.renderAdvertiseAsync = function (onAdvertiseRendered) {
            googletag.cmd.push(function () {
                ...
                googletag.display(_this.ContainerId);
        ...
        _this.setSlotRenderEndedEvent = function (delegate) {
            if (typeof delegate === 'function') {
                googletag.pubads().addEventListener('slotRenderEnded', delegate);
            }
        };

        _this.createSlot = function () {
            _this.Slot = googletag.defineSlot(_this.SlotName, [], _this.ContainerId);
            if (_this.Slot) {
                _this.Slot.ContainerId = _this.ContainerId;
                _this.Slot.addService(googletag.pubads());
                _this.Slot.setCollapseEmptyDiv(_this.CollapseEmptyDivs);
                _this.setTargeting();
                _this.setResponsive();
            } else {
        ...
        _this.refreshAd = function () {
            if (_this.GroupKey) {
                ...
                googletag.pubads().refresh([_this.Slot]);
            }
        };
        ...
        _this.destroySlot = function () {
            googletag.destroySlots([_this.Slot]);
        };
    ...
    FXStreet.Class.AdvertisePopup = function () {
        ...
        _this.init = function (json) {
            console.log("Creating AdvertisePopup");
            _this.setSettingsByObject(json);
            _this.setVars();

            if (!_this.getCookie(_this.CookieName)) {
                console.log("Showing AdvertisePopup");
                parent.init(json);
                if ($.browser.mobile) {
                    if (!_this.getCookie(_this.MobileCookieName)) {
        ...
        _this.setCookie = function () {
            ...
            setCookie(_this.CookieName, true, { expires: cacheExpirationInDays, path: '/' });
        }
        ...
        _this.setMobileCookie = function () {
            var cacheExpirationInDays = _this.PopupShowIntervalInMinutes / 24 / 60;
            setCookie(_this.MobileCookieName, true, { expires: cacheExpirationInDays, path: '/' });
        }

        _this.removeMobileCookie = function () {
            setCookie(_this.CookieName, "", { expires: cacheExpirationInDays, path: '/' });
        }
...
(function () {
    ...
    var getSize = function () {
        ...
        if (windowWidth <= widthLimitPx.mobile) {
            result = 'mobile';
        }
        else if (windowWidth <= widthLimitPx.tablet) {
            result = 'tablet';
        }
        else if (windowWidth <= widthLimitPx.desktop) {
            result = 'desktop';
        }
        return result;
    }
    ...
    var getLang = function () {
        ...
        switch (host) {
            case 'www.fxstreet.com':
            case 'qa.fxstreet.com':
                result = 'en';
                break;
            case 'www.fxstreet.es':
            case 'qa.fxstreet.es':
                result = 'es';
                break;
            case 'www.fxstreet.web.id':
            case 'qa.fxstreet.web.id':
                result = 'id';
                break;
            case 'www.fxstreet.ru.com':
            case 'qa.fxstreet.ru.com':
                result = 'ru';
                break;
            case 'www.fxstreet.de.com':
            case 'qa.fxstreet.de.com':
                result = 'de';
                break;
            case 'www.fxstreet.hk':
            case 'qa.fxstreet.hk':
                result = 'hk';
                break;
            case 'www.fxstreet.hu':
            case 'qa.fxstreet.hu':
                result = 'hu';
                break;
            case 'ar.fxstreet.com':
                result = 'ar';
                break;
            case 'www.fxstreet.fr':
            case 'qa.fxstreet.fr':
                result = 'fr';
                break;
            case 'www.fxstreet.com.vn':
            case 'qa.fxstreet.com.vn':
                result = 'vn';
                break;
            case 'www.fxstreet.jp':
            case 'qa.fxstreet.jp':
                result = 'jp';
                break;
            case 'www.fxstreet.com.tr':
            case 'qa.fxstreet.com.tr':
                result = 'tr';
                break;
            case 'www.fxstreet.it':
            case 'qa.fxstreet.it':
                result = 'it';
                break;
            case 'pt.fxstreet.com':
                result = 'pt';
                break;
            default:
                result = 'en';
    ...
    var getSection = function () {
        ...
        else if (pathname.startsWith('/economic-calendar') || pathname.startsWith('/calendario-economico')) {
            result = 'economic-calendar';
        }
        else if (pathname.startsWith('/cryptocurrencies')) {
            result = 'cryptocurrencies';
        }
        else if (pathname.startsWith('/live-video')) {
            result = 'live-video';
        }
        else if (pathname.startsWith('/rates-charts')) {
            result = 'rates-charts';
        }
        else if (pathname.startsWith('/education')) {
            result = 'education';
        }
        else if (pathname.startsWith('/brokers')) {
            result = 'brokers';
        }
    ...
    var cookieName = 'PopupAd_roadblocks';
    var cookiePolicy = 'policyAccepted';
    ...
    var showPopup = function () {
        var result = !$.browser.mobile
            && !cookieHelpers.get(cookieName)
            && window.location.pathname !== '/info/contest';
        return result;
    };

    var showPolicyPopup = function () {
        var result = !cookieHelpers.get(cookiePolicy) && window.location.hostname.startsWith('qa');
        return result;
    };

    var renderPrestitial = function () {
        var prestitialLandingPage = "https://staticconent.blob.core.windows.net/website/ads_popup/home.html?size="
            + getSize() + "&lang=" + getLang() + "&section=" + getSection();

        var prestitialTimeout = 15000;

        document.getElementsByTagName("body")[0].style.overflow = "hidden";
        document.write('<section id="prestitial-handle" class="fxs_prestitial-backdrop-darker fxs_prestitial-window-block" onClick="javascript:dismissPrestitial();"> <div class="fxs_prestitial-container"> <div class="fxs_prestitial-header"> <div class="fxs_prestitial-skip"> <div class="fxs_prestitial-continue" onclick="javascript:dismissPrestitial();">CONTINUE TO SITE</div> </div> </div> <iframe class="fxs_prestitial-iframe" style="overflow:hidden;" scrolling="no" frameborder="0" src="' + prestitialLandingPage + '"></iframe> </div> </section>');

        cookieHelpers.set(cookieName, true, 6);

        setTimeout(dismissPrestitial, prestitialTimeout);
    };

    window.dismissPrestitial = function () {
        var a = document.getElementById("prestitial-handle");
    ...
    var renderPolicy = function () {
        ...
        window.__cmp('init', {
            ...
            'Publisher Name': 'FXStreet',
            'Publisher Logo': 'https://staticcontent.fxstreet.com/website/img/logos/logo_notagline_white.png',
            'Display UI': 'always',
        ...
        __cmp('setConsentUiCallback', function () {
            cookieHelpers.set(cookiePolicy, true, 720);
        });
    };

    window.generatePopup = function () {
        if (showPolicyPopup()) {
            renderPolicy();
        }
        else if (showPopup()) {
